...
1.  **Omissão de Conteúdo Comercial e Contratação "No Escuro" (Banners Principais):** Ao navegar via tecla `Tab` pelas seções promocionais iniciais dos cartões Carrefour, Atacadão e Sam's Club, o seletor salta por cima de todas as chamadas explicativas. O NVDA cai diretamente em botões genéricos como "Saiba mais", ocultando o propósito real das ofertas para usuários cegos e gerando quebra severa na árvore de acessibilidade.
